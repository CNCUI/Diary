...
//固定电话，数字加'-'的组合
var TEL_REG = /^\d+(\-\d+)?$/;
//用户名，字母、下划线、数字组成 7到30字母或者数字
var USER_ACCOUNT = /^[0-9a-zA-Z]{7,30}$/;
//邮政编码
var POST_REG =/^[0-9]\d{5}$/;
// 整数
...
$(function() {
	...
	jQuery.validator.addMethod("email", function(value,element) {   
		return this.optional(element) || EMAIL_REG.test(value);   
	}, "请输入有效的电子邮箱"); 
	
	jQuery.validator.addMethod("postCode", function(value,element) {   
		return this.optional(element) || POST_REG.test(value);   
	}, "请输入有效的邮编"); 
	
	jQuery.validator.addMethod("useAccount", function(value,element) {   
		return this.optional(element) || USER_ACCOUNT.test(value);   
	}, "请输入有效的用户名，用户名由英文，数字或下划线组成"); 
...
TRIG.Validate = function(formid, submtfun, rules, ignore) {
	$("#" + formid).validate({
		...
		,invalidHandler : function() {
			TRIG.messager.alertError('数据验证不正确，请检查输入数据');
		}
		,errorPlacement : function(error, element) {
			//TRIG.messager.alert('['+$(element).attr('name')+']' + $(error).html());
			var icon = $(element);
