...
TRIG.PHOTO = TRIG.PHOTO || {};

TRIG.PHOTO = {
		_getCutUrl : function() {
			return TRIG.PATH + "/tools/photo_cut.json?token=" + getToken();
		},
		_getFileUrl : function() {
			return TRIG.PATH + "/tools/photo_getFileName.json?token=" + getToken();
		},
		did: 'photo',
		sid: '',
		nid: 'photoval',
		...
		cut : function(did, sid, nid, width, height) {
			...
			// 初始SWF
			var fo = new FlashObject(TRIG.PATH + "/swf/photo.swf", "photoFlash", this.width, this.height , "10", "#ffffff");
			...
			$.get(this._getFileUrl(),"",function(data) {
				...
				if(data.success==true) {
					if(_this.sid&&document.getElementById(_this.sid)) {
						document.getElementById(_this.sid).src = TRIG.PATH + data.img;
					}
		...
		startInterval: function() {
			this.interval = setInterval("TRIG.PHOTO.getFileName()", 1000);
		},
