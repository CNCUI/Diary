 ...
 */
TRIG.search = function(tablename, searchobj, ttable, aliasname, fordata) {
	if(TRIG.isPrivlege("findSysSearch") == false) { 
		return false;
	}
	...
	TRIG.LOG.TableName = tablename;
	$.getJSON(TRIG.PATH + '/tools/table_column_findList.json', 'stc_tableName='+tablename+'&sort=stc_pageName&stc_isSearch=Y',function(data) {
	...
	$("#AdvancedSearchbbBtn").bind('click',function(){
		if($("#searchtext1").val()!=''){
			var sqlwhere = TRIG._getSearchWhere();
			var querydata = {
					where: sqlwhere,
...
$(function(){
	...
	$("#AdvancedSearchSaveBtn").bind('click', function(){
		if($("#searchtext1").val()!=''){
			...
			TRIG.messager.alert("没有条件");
		}
	});
	
	TRIG.Validate('searchEditform', function(form){
		var id = $("#ss_searchId").val();
		var _url = TRIG.PATH + '/search_add.json';
		if(id!='') {
			_url = TRIG.PATH + '/search_update.json';
		}
		$("#wheres").val(TRIG._getSearchWhere());
	...
	$("#AdvancedSearchOpenBtn").bind('click', function(){
		if(pttable==undefined) {
			pttable = new TRIG.Datatable({
				table : $("#trig-pro-search-table"), 
				url : TRIG.PATH + '/search_findPageList.json' , 
				columns : [
		...
		if(sttable == undefined) {
			sttable = new TRIG.Datatable({
				table : $("#trig-share-search-table"), 
				url : TRIG.PATH + '/search_findPageList.json?t=1', 
				columns : [
				           { "data": null, "render": function(data, type, row, mete){ 
	...
	function setWhere(id) {
		$.post(TRIG.PATH + '/search_findWhere.json', 'id='+ id, function(data){
	...
	$("#deleteSysSearch").bind('click', function() {
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(TRIG.PATH + '/search_delete.json', "ids=" + ids, function(data){
				pttable.reload();
			});
		});
	});
