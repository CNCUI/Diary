$(function(){
    ...
    var ttable = new TRIG.Datatable({
		...
		dblclick : function() {   // 双击打开编辑浮层
			// 是否有修改权限
	    	if(TRIG.isPrivlege("updateSysOrganization")) {
	    		showUpdate();
	    	}
		} ,
		dnd: {	// 拖动排序
			...
			onDrop: function(id, previd, nextid){
	 			...
	 			if(moveid)
		 			TRIG.trackPost(organization.url.moveSort,'id=' + id + '&moveId=' + moveid, function(data){
							ttable.reload();
							refreshtree();
					});
			}
    ...
    $( "#org_tree" ).on('move_node.jstree', function(e, data){
        ...
        TRIG.trackPost(organization.url.updateSort, {
    ...
    // 编辑
    // 初始类别选择框 
    TRIG.dict.setSelectByNO('so_type','ORGANIZATION_TYPE'); 
    // 新增
    $("#addSysOrganization").bind('click',function(){
    	TRIG.formreset($("#orgEditform"));
    	TRIG.hidePrveNext();
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#orgEditform"), data);
			$("#orgEditModal").modal('show');
		}
    }
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_organization", "orgEditform",  function (form) {
    ...
    $("#deleteSysOrganization").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(organization.url.del, "ids=" + ids, function(data){
				ttable.reload();
				refreshtree();
			});
		});
    ...
    $(".findSysOrganizationRole").bind('click', function(){
    	...
    	if(roleMultiSelect != null) {
			...
			$.getJSON("organization_role_findRole.json", "", function(data){ 
				...
				roleMultiSelect = new TRIG.multiSelect('orgroleselect', datas);  
				setOrgRole(); 
			});
    	}
    });
    	...
    	$.post('organization_role_deletes.json','sor_organizationId=' + orgid ,function(data) { 
			...
			if(roles) {
				$.post('organization_role_addRole.json', 'id='+orgid+'&ids=' + roles, function(data2){  
					data2 = TRIG.successHandle(data2);
					if(data2.success) {
						TRIG.updateSuccessAlert(); 
    ...
    $(".moveSysOrganization").bind('click', function(){
    	TRIG.trackPost(organization.url.sorts,"id=" + pid, function(data){
				refreshtree();
				ttable.reload(); 
		});
    });
    ...
    $("#moveSave").bind('click', function(){
    	...
    	if(moveId) {
	    	...
	    	TRIG.trackPost(organization.url.move,'id=' + selfid + "&moveId=" + moveId,function(data){
					ttable.reload();
					$('#org_tree').jstree('refresh_node', {id: '2'});   
					$("#orgmoveselect").val('');
