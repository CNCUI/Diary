$(function(){
    ...
    // 高级查询
    TRIG.search('sys_exception_log', $("#findSysExceptionLog"), ttable);  
    ...
    $("#deleteSysExceptionLog").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysExceptionLog.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
    });
    ...
    $("#deletesSysExceptionLog").bind('click', function(){
    	TRIG.isDels(function(){
			...
			TRIG.trackPost(sysExceptionLog.url.dels,para,function(data){
					ttable.reload();
			});
