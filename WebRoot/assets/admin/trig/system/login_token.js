$(function(){
    ...
    // 高级查询
    TRIG.search('sys_login_token', $("#findSysLoginToken"), ttable);  
    ...
    ttable.initMore(function(id) { 
   	...
   	if (rowData.slt_loginType == '1') {
   		typestr = TRIG.TEXT.LOGIN.type1;
   	} else if(rowData.slt_loginType== '2') {
   		typestr = TRIG.TEXT.LOGIN.type2;
	}
	else {
		typestr = TRIG.TEXT.LOGIN.type0;
	}
   	 var sOut = '<table>';
    ...
    $("#deleteSysLoginToken").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(loginToken.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
    });
