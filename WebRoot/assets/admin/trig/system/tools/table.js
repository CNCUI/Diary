$(function(){
    ...
    var ttable = new TRIG.Datatable({
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysTable")) { 
    	    		showUpdate();
    	    	}
    		} 
    });
    ...
    // 高级查询
    TRIG.search('sys_table', $("#findSysTable"), ttable);  
    
    // 新增浮层初始
    $("#addSysTable").bind('click', function(){
    	TRIG.formreset($("#tableEditform"));
    	TRIG.hidePrveNext();
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#tableEditform"), data);
    ...
    // 是否点击保存&关闭
    var saveclose = false;
    TRIG.FormValidate("sys_table", "tableEditform",  function (form) {
	...
	$("#deleteSysTable").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysTable.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
	});
	...
	// 日志
	 ...
	 var valis = [];
	 $.getJSON(TRIG.PATH+ '/dictionary_findByNoList.json?id=validator', '', function(data) {
	...
	function editRow(id, nRow) {
			...
			if(TRIG.isSelectOne(rows)) {
				...
				$("#tablename").text(tablename);
				 if(columnttable == undefined) {
					 columnttable = new TRIG.Datatable({
					 		table : columntable, 
	 ...
	 function addinit() {
		 TRIG.trackPost(sysTable.column.url.addInit, "stc_tableId="+tableid +
	 ...
	 $("#addSysTableColumn").bind('click', function(){
			if(initb==true) {
				TRIG.messager.confirm(TRIG.TEXT.OTHER.RELOAD, TRIG.TEXT.OTHER.COLUMN_EXIST_RELOAD, function(r){
					if(r) {
						addinit();
					}
				});
	 ...
	 $("#updateSysTableColumn").bind('click', function(){
		 TRIG.trackPost(sysTable.column.url.add, "stc_tableId="+tableid +
					"&stc_tableName=" + tablename, function(data){
				columnttable.reload();
			});
	 }) ;
	 ...
	 $("#deleteSysTableColumn").bind('click', function(){
		 var objs = columnttable.getCheckbox();
			...
			TRIG.isDelSelectOne(rows,function(){
				...
				TRIG.trackPost(sysTable.column.url.del, "ids=" + ids, function(data){
					columnttable.reload();
				});
			});
	 });
