function sqltoolsexp(type) {
	...
	var sql = $("#expsql").val();
	if(sql!="") {
		$("#frcs").attr('name', TRIG.Contants.CSRF_PARA);
		$("#frcs").val(getCookie(TRIG.Contants.CSRF_COOKIE));
		document.getElementById("expform").submit();
	}else{
		TRIG.messager.alert("请输入SQL并正确执行");
	}
}
...
$(function() {
	...
	$("#backupSql").bind('click', function(){
		TRIG.messager.confirm('备份？','确认备份？', function(r) {
			if(r) {
				TRIG.trackPost(sqltools.url.backup, '', function(data) {
					TRIG.messager.alert("备份成功");
				});
			}
		});
	});
	...
	$("#sqlfile_fb").bind('click', function(){
		TRIG.UPLOAD.open('sqlfile', TRIG.FILETYPE_FILE, '/WEB-INF/backup/sql');
	});
	var sf = new TRIG.FileSelect('/WEB-INF/backup/sql', 'sqlfile');
	$("#sqlfile_select").bind('click', function(){
			sf.open();
	});
	
	$("#exceuteScriptSqlbtn").bind('click', function(){
		TRIG.messager.confirm('恢复？','确认恢复？', function(r) {
			if(r) {
				var id = $("#sqlfile").val();
				TRIG.trackPost(sqltools.url.recover, 'id=' + id, function(data) {
					TRIG.messager.alert("恢复成功");
					$("#sqlfile").val('');
				});
			}
		});
	...
	TRIG.Validate('sqlEditform', function(form){
		...
		if($.trim(sql).toUpperCase().StartWith('SELECT')) {
			...
			$.post(sqltools.url.updateSQL, 'sql=' + sql, function(data) {
				...
				if(data.success==false) {// 失败
					...
					$("#expsql").val(sql);
					TRIG.messager.alert("更新行数：" + data.num);
				}
			});
		}
	});
	...
	var sqltablerows = [];
	var pttable = new TRIG.Datatable({
	...
	var sttable = new TRIG.Datatable({
	...
	$("#sqlselectbtn").bind('click', function(){
		...
		if(index=='0') {
			...
			if(TRIG.isSelectOne(rows)) {
				...
				$("#expsql").val('');
				TRIG.forminit($("#sqlEditform"), rowdata);
	...
	$("#deleteSqlTools").bind('click', function() {
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sqltools.url.del, "ids=" + ids, function(data){
				pttable.reload();
			});
		});
	});
		...
		if(sql!="") {
			$.post(sqltools.url.list + "?sql=" + sql,'rows=0', function(data){
				...
				if(data.data.length>0) {
					...
					TRIG.Chart('container', $("#sst_sqlName").val(), data, type);
				} else {
