...
$(function(){
    ...
    // begin first table
    trig_cache_table = new TRIG.Datatable({
    		table : table, 
    		url : cache.url.find, 
    		columns : [
    ...
    TRIG.Validate('cacheRemoveform', function(form){
    	TRIG.trackPost(cache.url.removeKey, $(form).serialize(), function(data){
    		TRIG.messager.alert("清除成功");
    		$("#cacheRemoveModal").modal('hide');
    		TRIG.formreset($("#cacheRemoveform"));
...
function trig_tools_cache_remove(id) {
	TRIG.trackPost('tools/cache_remove.json', 'id=' + id, function(data){
		TRIG.messager.alert("清除成功");
		trig_cache_table.reload();
	});
}
