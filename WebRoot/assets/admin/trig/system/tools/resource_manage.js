$(function(){
    ...
    TRIG.Validate('createform', function(form){
    	TRIG.trackPost(resource.url.create, $(form).serialize(), function(data){
				refreshtree();
				ttable.reload(); 
    ...
    $(".pasteResourceManage").bind('click', function(){
    	if(resource.copyids == ""){
			TRIG.messager.alert("没有粘贴的文件");
			return false;
		}
    	TRIG.progress();
		$.post(rTRIG.trackPoste.url.copy,"fileName=" + resource.copyids + "&id=" + pid +"&copydir=" + resource.copydirid
    ...
    $("#downloadResourceManage").bind('click', function(){
		...
		if(TRIG.isSelectOne(rows)) {
			...
			if(rowData.type!='Folder') {
				document.getElementById("uploadiframe").src = resource.url.download + "?id=" + pid + "&fileName=" + id+ "&" + TRIG.Contants.CSRF_PARA + "=" + getCookie(TRIG.Contants.CSRF_COOKIE);
			} else {
    ...
    $("#deleteResourceManage").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(resource.url.del, "fileName=" + ids+"&id=" + pid, function(data){
				ttable.reload();
				refreshtree();
			});
		});
    ...
    $("#deleteAllResourceManage").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(resource.url.delAll, "fileName=" + ids+"&id=" + pid, function(data){
				ttable.reload();
				refreshtree();
			});
		});
		...
		if(rows>0) {
			TRIG.messager.confirm('备份','确认备份？', function(r) {	
				if(r) {
					...
					TRIG.trackPost(resource.url.backup, "fileName=" + ids+"&id=" + pid, function(data) {
						TRIG.messager.alert("备份成功，WEB-INF/backup/file 下载备份。");
					});
				}
			});
		} else {
