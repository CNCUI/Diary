$(function(){
    ...
    var ttable = new TRIG.Datatable({
		...
		dblclick : function() {   // 双击打开编辑浮层
			// 是否有修改权限
	    	if(TRIG.isPrivlege("updateSysModuleFunction")) {
	    		showUpdate();
	    	}
		} 
    });;
    ...
    $("#addSysModuleFunction").bind('click',function(){
    	TRIG.formreset($("#module_functionEditform"));
    	TRIG.hidePrveNext();
 ...
 // 修改加载表单数据
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#module_functionEditform"), data);
			$("#module_functionEditModal").modal('show');
		}
    }
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_module_function", "module_functionEditform",  function (form) {
    ...
    $("#deleteSysModuleFunction").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(modulefunction.url.del, "ids=" + ids, function(data){
				ttable.reload();
				refreshtree();
			});
		});
    ...
    $(".moveSysModuleFunction").bind('click', function(){
    	TRIG.trackPost(modulefunction.url.sorts,"id=" + pid, function(data){
				refreshtree();
				ttable.reload(); 
		});
    });
