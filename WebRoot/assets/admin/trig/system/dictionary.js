$(function(){
    ...
    var ttable = new TRIG.Datatable({
		...
		dblclick : function() {   // 双击打开编辑浮层
			// 是否有修改权限
	    	if(TRIG.isPrivlege("updateSysDictionary")) {
	    		showUpdate();
	    	}
		}  ,
		dnd: {	// 拖动排序
			...
			onDrop: function(id, previd, nextid){
	 			...
	 			if(moveid)
	 				TRIG.trackPost(TRIG.getContentUrl(dictionary.url.moveSort, pid),'id=' + id + '&moveId=' + moveid, function(data){
							ttable.reload();
							refreshtree();
					});
			}
    ...
    $( "#dictionary_tree" ).on('move_node.jstree', function(e, data){
        ...
        TRIG.trackPost(TRIG.getContentUrl(dictionary.url.updateSort, pid), {
    ...
    function formreset() {
    	TRIG.formreset($("#dictionaryEditform"), {sd_isLeaf : 'Y', sd_state : 'N'});
    }
 ...
 // 修改加载表单数据
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#dictionaryEditform"), data);
			$("#dictionaryEditModal").modal('show');
		}
    }
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_dictionary", "dictionaryEditform",  function (form) {
    ...
    $("#deleteSysDictionary").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(TRIG.getContentUrl(dictionary.url.del, pid), "ids=" + ids, function(data){
				ttable.reload();
				refreshtree();
			});
		});
    ...
    $(".moveSysDictionary").bind('click', function(){
    	TRIG.trackPost(TRIG.getContentUrl(dictionary.url.sorts, pid),"id=" + pid, function(data){
				refreshtree();
				ttable.reload(); 
		});
    });
    ...
    $("#moveSave").bind('click', function(){
    	...
    	if(moveId) {
	    	if(selfid==moveId || moveId==tablerows[selfid].sd_parentDictionaryId) {
				TRIG.messager.alert(TRIG.TEXT.PUBLIC.SORT.NOTMOVE);
				return false;
			}
	    	TRIG.trackPost(TRIG.getContentUrl(dictionary.url.move, pid),'id=' + selfid + "&moveId=" + moveId,function(data){
				ttable.reload();
				$('#dictionary_tree').jstree('refresh_node', {id: '1'});   
    ...
    $(".dictionarySort").bind('click', function(){
    	...
    	if(TRIG.isSelectOne(rows)) {
    		...
    		TRIG.forminit($("#dictionarySortEditform"), rowData);
    		$("#dictionarySortModal").modal('show');
    	}
    });
    
    TRIG.Validate("dictionarySortEditform",  function (form) {
			TRIG.trackPost(TRIG.getContentUrl(dictionary.url.updateSort, pid), $(form).serialize(), function(data){
				ttable.reload();
    ...
    $("#updateAddRolePrivlegeDef").bind('click', function(){
    	TRIG.trackPost(TRIG.getContentUrl(dictionary.url.updateContentManager, pid), '', function(data){
    		TRIG.updateSuccessAlert(); 
		});
    });
});