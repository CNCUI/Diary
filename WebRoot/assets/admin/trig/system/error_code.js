$(function(){
    ...
    var ttable = new TRIG.Datatable({
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysErrorCode")) { 
    	    		showUpdate();
    	    	}
    		} 
    });
    ...
    // 高级查询
    TRIG.search('sys_error_code', $("#findSysErrorCode"), ttable);  
    
    // 新增浮层初始
    $("#addSysErrorCode").bind('click', function(){
    	TRIG.formreset($("#errorCodeEditform"));
    	TRIG.hidePrveNext();
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#errorCodeEditform"), data);
			$("#isupdate").val('Y');
    ...
    // 是否点击保存&关闭
    var saveclose = false;
    TRIG.FormValidate("sys_error_code", "errorCodeEditform",  function (form) {
	...
	$("#deleteSysErrorCode").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysErrorCode.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
	});
