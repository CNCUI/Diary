$(function(){
    ...
    var ttable = new TRIG.Datatable({
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysIpList")) { 
    	    		showUpdate();
    	    	}
    		} 
    });
    ...
    // 高级查询
    TRIG.search('sys_ip_list', $("#findSysIpList"), ttable);    
    
    // 新增浮层初始
    $("#addSysIpList").bind('click', function(){
    	TRIG.formreset($("#ipEditform"));
    	TRIG.hidePrveNext();
    ...
    // 是否点击保存&关闭
    var saveclose = false;
    TRIG.FormValidate("sys_ip_list", "ipEditform",  function (form) {
	...
	$("#deleteSysIpList").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysIpList.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
	});
