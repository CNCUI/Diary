$(function(){
    ...
    var ttable = new TRIG.Datatable({
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysUserGroup")) { 
    	    		showUpdate();
    	    	}
    		} 
    });
    ...
    // 高级查询
    TRIG.search('sys_user_group', $("#findSysUserGroup"), ttable); 
    
    // 新增浮层初始
    $("#addSysUserGroup").bind('click', function(){
    	TRIG.formreset($("#userGroupEditform"));
    	TRIG.hidePrveNext();
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#userGroupEditform"), data);
			$("#userGroupEditModal").modal('show');
		}
    }
    ...
    // 是否点击保存&关闭
    var saveclose = false;
    TRIG.FormValidate("sys_user_group", "userGroupEditform",  function (form) {
	...
	$("#deleteSysUserGroup").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysUserGroup.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
	});
    ...
    $(".findSysUserGroupRole").bind('click', function(){
    	...
    	if(roleMultiSelect==null) {
    		$.getJSON("organization_role_findRole.json", "", function(data){ 
    			...
    			roleMultiSelect = new TRIG.multiSelect('usergrouproleselect', datas);
    			setGroupRole(); 
    		});
    	}else { 
    		setGroupRole();
    ...
    $("#userGroupRoelSaveBtn").bind('click', function(data){ 
    	$.post('user_group_role_deletes.json','sugr_groupId=' + groupid ,function(data) { 
			...
			if(roles) {
				TRIG.trackPost('user_group_role_add.json', 'id='+groupid+'&ids=' + roles, function(data2){  
						TRIG.updateSuccessAlert(); 
    ...
    $("#addSysUserGroupUser").bind('click', function(){
    	...
    	if(uids) {
    		TRIG.trackPost('user_group_user_add.json','id='+groupid + "&ids=" + uids, function(data){
					userttable.reload(); 
    ...
    $("#deleteSysUserGroupUser").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost('user_group_user_delete.json', "ids=" + ids, function(data){ 
				userttable.reload();
			});
		});
    });
