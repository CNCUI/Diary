$(function(){
    ...
    // 高级查询
    TRIG.search('sys_function_log', $("#findSysFunctionLog"), ttable);  
    ...
    $("#deleteSysFunctionLog").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysFunctionlog.url.del, "ids=" + ids, function(data){
				ttable.reload();
			});
		});
    });
    ...
    $("#deletesSysFunctionLog").bind('click', function(){
    	TRIG.isDels(function(){
			...
			TRIG.trackPost(sysFunctionlog.url.dels,para,function(data){
					ttable.reload();
			});
		});
    });
