$(function(){
    ...
    var ttable = new TRIG.Datatable({
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysRole")) {
    	    		showUpdate();
    	    	}
    		},
    		paging: false 
 ...
 // 显示更多字段值
    ...
    // 高级查询
    TRIG.search('sys_role', $("#findSysRole"), ttable); 
    
 // 新增浮层初始
    $("#addSysRole").bind('click', function(){
    	TRIG.formreset($("#roleEditform"));
    	TRIG.hidePrveNext();
    	$("#sr_isLocked").val('N'); 
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#roleEditform"), data);
			$("#roleEditModal").modal('show');
			// 加记录锁
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_role", "roleEditform",  function (form) { 
			...
			if($("#sr_roleId").val()!='') {
				...
				if(tablerows[ $("#sr_roleId").val()].sr_roleType == role.type.nz) {
					TRIG.messager.alert(tablerows[ $("#sr_roleId").val()].sr_roleName + '为系统内置角色，不能修改。'); 
					return false;
				}
			}
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			objs.each(function(){
				...
				if(tablerows[ this.value].sr_roleType == role.type.nz) {
					TRIG.messager.alert(tablerows[ this.value].sr_roleName + '为系统内置角色，不能删除。'); 
					isnz = true;
					return false;
				}
			});   
			if(isnz==false) {
				TRIG.trackPost(role.url.del, "ids=" + ids, function(data){ 
					ttable.reload();
				});
			}
		});
    ...
    $("#addUserRole").bind('click', function(){
    	...
    	if(uids) {
    		TRIG.trackPost('user_role_addUser.json','id='+roleid + "&ids=" + uids, function(data){
					userttable.reload(); 
    ...
    $("#deleteUserRole").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost('user_role_delete.json', "ids=" + ids, function(data){ 
				userttable.reload();
			});
		});
    });
		    	...
		    	$.getJSON('role_privlege_def_findList.json', 'srpd_roleId=' + roleid, function(data){  
		    		...
		    		if(document.getElementById("trig-role-content-table")) {
		    			if(contentttable == undefined) {
			    			contentttable = new TRIG.Datatable({
		    			...
		    			if(document.getElementById("trig-role-column-table")) {
		    				if(columntable==undefined) { // 初始
		    					columntable = new TRIG.Datatable({
    		...
    		if(tabindex=="0") {
		    	...
		    	$.post('role_privlege_def_add.json','type=0&id='+roleid+'&ids='+ids, function(data){
					...
					if(data.success==true) {
						ids = TRIG.TreeUndetermined('tree_role_priv'); 
    ...
    $("#deleteRoleContent").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(rolecontent.url.del, "ids=" + ids, function(data){ 
    ...
    // 内容权限选择框初始
    TRIG.dict.setSelectByNOAndCode('src_contentPrivlegeDef','CONTENT_PRIV');
    ...
    $("#addRoleContent").bind('click', function(){
    	TRIG.formreset($("#contentPrivAddform"));
    	$("#src_roleId").val(roleid);
    	$("#src_contentId").select2('val', '');   
    	$("#contentPrivAddModal").modal('show');
    });
    // 保存内容权限
    TRIG.Validate('contentPrivAddform', function(form){
    	TRIG.trackPost(rolecontent.url.add, $(form).serialize(), function(data){
    		contentttable.reload();
    ...
    $("#addRoleNotColumn").bind('click', function() {
    	...
    	if(cid) {
			...
			TRIG.trackPost('role_not_column_add.json','srnc_roleId='+roleid + "&srnc_tableName=" + st_tableName
    ...
    $("#deleteRoleNotColumn").bind('click', function() {
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost('role_not_column_delete.json', "ids=" + ids, function(data){ 
				columntable.reload();
			});
		});
    });
    ...
    $(".LockedYes").bind('click', function(){ 
    	...
    	if(TRIG.isSelectOne(rows)) {
    		var id = ids.get(0).value;
    		TRIG.trackPost(role.url.updatelocked,'sr_isLocked=Y&sr_roleId=' + id,function(data){
    				TRIG.updateSuccessAlert();
    				ttable.reload();
    		});
    	}
    ...
    $(".LockedNo").bind('click', function(){
    	...
    	if(TRIG.isSelectOne(rows)) {
    		var id = ids.get(0).value;
    		TRIG.trackPost(role.url.updatelocked,'sr_isLocked=N&sr_roleId=' + id,function(data){
    				TRIG.updateSuccessAlert();
    				ttable.reload();
    		});
    	}
    ...
    table.on('click', 'tbody tr', function (e) {
    	...
    	if(c.is(":checked")) {
    		...
    		if(TRIG.isPrivlege("updateRoleLocked")){
