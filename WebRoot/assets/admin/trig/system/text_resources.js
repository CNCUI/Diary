$(function(){
    ...
    var ttable = new TRIG.Datatable({
		...
		dblclick : function() {   // 双击打开编辑浮层
			// 是否有修改权限
	    	if(TRIG.isPrivlege("updateTextResources")) {
	    		showUpdate();
	    	}
		} 
    });
    ...
    $("#addTextResources").bind('click',function(){
		if(pid=='' || pid=='0') {
			TRIG.messager.alert(TRIG.TEXT.PUBLIC.SELECT_PLEASE);
			return ;
		}
    	TRIG.formreset($("#textResourcesEditform"));
    	TRIG.hidePrveNext();
 ...
 // 修改加载表单数据
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#textResourcesEditform"), data);
			$("#textResourcesEditModal").modal('show');
		}
    }
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_text_resources", "textResourcesEditform",  function (form) {
    ...
    $("#deleteTextResources").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysTextResources.url.del, "ids=" + ids, function(data){
				ttable.reload();
				refreshtree();
			});
		});
