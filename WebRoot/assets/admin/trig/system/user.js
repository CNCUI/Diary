$(function() {
	...
	var user = function () {
			...
			var tableInit = function() {
				ttable = new TRIG.Datatable({
			    		...
			    		dblclick : function() {   // 双击打开编辑浮层
			    			// 是否有修改权限
			    	    	if(TRIG.isPrivlege("updateSysUser")) {
			    	    		updateOpen();
			    	    	}
			    		},
			    		filter: true 
			...
			var updateOpen = function() {
				...
				if(TRIG.isSelectOne(rows)) {
					...
					// 初始表单数据
					TRIG.forminit($("#userEditform"), data);
			...
			var formSaveInit = function() {
				...
				if(TRIG.settings.org) {
					...
					TRIG.OrgSelect('su_organizationId', '', '', orgs);
				}
				
				// 是否点击保存&关闭
			    var saveclose = false;
				TRIG.FormValidate("sys_user", "userEditform",  function (form) {
						var _url = url.add;
						var isadd = true;
			...
			var lockedInit = function() {
			    ...
			    table.on('click', 'tbody tr', function (e) {
			    	...
			    	if(c.is(":checked")) {
			    		...
			    		if(TRIG.isPrivlege("updateUserLocked")){
			...
			return {
				...
				lockedYes : function() {
			    	...
			    	if(TRIG.isSelectOne(rows)) {
			    		...
			    		TRIG.trackPost(url.updatelocked,'su_isLockedOut=Y&su_userId=' + uid,function(data){
			    				TRIG.updateSuccessAlert();
			    				ttable.reload();
			    		});
			    	}
				...
				lockedNo : function() {
			    	...
			    	if(TRIG.isSelectOne(rows)) {
						...
						var rowData = tablerows[id];
			    		var uid = rowData.su_userId;
			    		TRIG.trackPost(url.updatelocked,'su_isLockedOut=N&su_userId=' + uid,function(data){
			    				TRIG.updateSuccessAlert();
			    				ttable.reload();
			    		});
			    	}
				...
				init : function() {
				    ...
				    // 导出
				    TRIG.initexpHTML('exportSysUser', 'user_export.t', ttable);
				    
				    // 高级查询
				    TRIG.search('sys_user', $("#findSysUser"), ttable);
	...
	var userpwd = function(ttable, tablerows){
		...
		return {
			...
			init : function() {
				TRIG.Validate('pwdform', function(){
			    	TRIG.trackPost(url.updatepwd, $("#pwdform").serialize(), function(data){
			    			TRIG.updateSuccessAlert();
			    			$("#pwdModal").modal('hide');  
			    	});
			    	return false; 
	    ...
	    return {
	        		...
	        		if(TRIG.isSelectOne(rows)) {
		        		...
		        		$.getJSON(url.findRole, "", function(data){
	        				...
	        				$.getJSON("user_role_findList.json", "sur_userId="+uid, function(data){
	        	    			...
	        	    			roleMultiSelect = new TRIG.multiSelect('roleselect', datas, initdatas);
	        	    			$("#roeModal").modal('show'); 
	        	    		});
	        			});
	        		}
	    	...
	    	save : function() {
	    		$.post(url.del,'sur_userId=' + uid ,function(data) { 
	    			...
	    			if(roles) {
	    				TRIG.trackPost(url.add, 'id='+uid+'&ids=' + roles, function(data2){ 
	    						TRIG.updateSuccessAlert(); 
	    						$("#roeModal").modal('hide');  
	    				});
	    			}else {
		    	...
		    	Pace.track(function() {
			    	...
			    	$.post(url.add,'type=0&id='+uid+'&ids='+ids, function(data){
						...
						if(data.success==true) {
							ids = TRIG.TreeUndetermined('tree_priv'); 
