$(function(){
	...
	var data_recycle_bin = function () {
			...
			return {
				// 查询
				query : function() {
					ttable.query({ sdrb_dataName : $("#dataname").val()});
				},
				// 删除
				del : function() {
				...
				update : function() {
					...
					if(TRIG.isSelect(rows)) {
						...
						TRIG.trackPost(url.update, "ids=" + ids, function(data){
							ttable.reload();
						});
					}
				},
				delAll: function() {
					TRIG.messager.confirm("确认清空？","确认清空？",function(r){
					//是否删除	
						if(r) {
							TRIG.trackPost(url.delAll,'', function(data){
								ttable.reload();
							});
						}
					});
