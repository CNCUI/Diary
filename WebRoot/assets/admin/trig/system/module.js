$(function(){
    ...
    var ttable = new TRIG.Datatable({
		...
		dblclick : function() {   // 双击打开编辑浮层
			// 是否有修改权限
	    	if(TRIG.isPrivlege("updateSysModule")) {
	    		showUpdate();
	    	}
		} ,
		dnd: {	// 拖动排序
			...
			onDrop: function(id, previd, nextid){
	 			...
	 			if(moveid)
		 			TRIG.trackPost(sysModule.url.move,'id=' + id + '&moveId=' + moveid, function(data){
							ttable.reload();
							refreshtree();
					});
			}
    ...
    $( "#module_tree" ).on('move_node.jstree', function(e, data){
        ...
        TRIG.trackPost(sysModule.url.updateSort, {
 ...
 // 修改加载表单数据
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#moduleEditform"), data);
			$("#moduleEditModal").modal('show');
		}
    }
    ...
    var saveclose = false;
    // 添加/编辑->保存
    TRIG.FormValidate("sys_module", "moduleEditform",  function (form) {
    ...
    $("#deleteSysModule").bind('click', function(){
		...
		TRIG.isDelSelectOne(rows,function(){
			...
			TRIG.trackPost(sysModule.url.del, "ids=" + ids, function(data){
				ttable.reload();
				refreshtree();
			});
		});
 ...
 // 角色
    ...
    $(".moveSysModule").bind('click', function(){
    	TRIG.trackPost(sysModule.url.sorts,"id=" + pid, function(data){
				refreshtree();
				ttable.reload(); 
		});
    });
