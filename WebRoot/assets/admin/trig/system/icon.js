$(function(){
    ...
    var ttable = new TRIG.Datatable({
    					   ...
    					   tablerows[row.si_iconId] = row;
    						return '<input type="checkbox" class="checkboxes" value="'+row.si_iconId+'"/>';   
    		                 ...
    		                 { "data": "si_iconPath", "render":function(value){
    		                	 if(value=='1') { //系统
    		 						return TRIG.TEXT.OTHER.typesys;
    		 					}else {
    		 						return TRIG.TEXT.OTHER.typeuser; //自定义
    		 					}
    		                 }},  
    		                 { "data": "si_iconType" }, 
    		                 { "data": "si_createTime" }
    		...
    		dblclick : function() {   // 双击打开编辑浮层
    			// 是否有修改权限
    	    	if(TRIG.isPrivlege("updateSysIcon")) { 
    	    		showUpdate();
    	    	}
    		} 
    });
    ...
    // 高级查询
    TRIG.search('sys_icon', $("#findSysIcon"), ttable);  
    
    // 新增浮层初始
    $("#addSysIcon").bind('click', function(){
    	TRIG.formreset($("#iconEditform"));
    	TRIG.hidePrveNext();
    ...
    function showUpdate() {
		...
		if(TRIG.isSelectOne(rows)) {
			...
			// 初始表单数据
			TRIG.forminit($("#iconEditform"), data);
			$("#iconEditModal").modal('show');
		}
    }
    ...
    // 是否点击保存&关闭
    var saveclose = false;
    TRIG.FormValidate("sys_icon", "iconEditform",  function (form) {
