...
TRIG.isRole = function(rolename) {
	return (trigroles + ",").indexOf(rolename + ",") > -1 || rolename=='All'; 
};
...
TRIG.forminit = function(form, data) {
	...
	for(var name in data) {
		...
		if (!rr.length) {
			...
			if($('select[name="'+name+'"]', form).size() > 0)  {
...
TRIG.getUserAvatar = function(data) {
	var un = TRIG.STATICPATH + "/frontend/assets/layouts/layout/img/male.png";
	if(data) {
		if(data.su_userAvatar) {
			un = TRIG.UPLOADPATH + data.su_userAvatar;
		} else {
			if(data.su_sex == 'F')
			un = TRIG.STATICPATH + "/frontend/assets/layouts/layout/img/female.png";
		}
	}
	return un;
};
...
TRIG.initUserDef = function(def) {
	if(def) {
		...
		if(def.sud_style) {
			...
			$('#style_color').attr("href",  TRIG.STATICPATH + '/assets/layouts/layout3/css/themes/' + TRIG.style + ".css");
		} 
...
TRIG.getJSON = function(url, data, callfun, async) {
	var oldasync = $.ajaxSettings.async;
	 $.ajaxSettings.async = (async == false ? false : true); // 默认异步true
	 $.getJSON(url, data, callfun);
...
TRIG.DataDeal = {
	// 将指定ID的form表单通过$('#formid').serialize()获取的值转成json字符串
   	formToJson: function (formId) {
   		var fid = formId ? formId : "#trig-search-form";
   		var data = decodeURIComponent($(fid).serialize(), true);
       	data = data.replace(/&/g,"\",\"");
       	data = data.replace(/=/g,"\":\"");
       	data = "{\""+data+"\"}";
       	return data;
