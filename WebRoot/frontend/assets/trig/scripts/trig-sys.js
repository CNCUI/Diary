...
TRIG.dict = {
	 url : {
		parent: TRIG.PATH+ '/frontend/dictionary_findByParentList.json',
		NO: TRIG.PATH+ '/frontend/dictionary_findByNoList.json',
		ByNO: TRIG.PATH + '/frontend/dictionary_findByNo.json'
	 	},
		clear : function(id) { 
			$('#' + id).val('');
		},
		...
		setSelectByParentAndCode:function(id, pid, defval) {
			...
			if(!defval) defval = jpobj.attr('data-value'); // 默认值或初始值
			$.getJSON(_url,'id=' + pid,function(data) {
				for(var i=0;i<data.length;i++) {
					var opt = new Option(data[i].sd_dictionaryName, data[i].sd_dictionaryCode);
					$(opt).attr('data-id', data[i].sd_dictionaryId);
					if(defval && defval == data[i].sd_dictionaryCode) {
						opt.selected = true;
		...
		setMultipleByParent: function(name, pid, divid, defval){
			...
			html += '</div>'; 
			$("#" + divid).html(html);
		},setSelectsByNOAndCode:function(ids, NO) {
			var _url = this.url.NO ;
			var id = ids[0];
			var opts = document.getElementById(id).options;
			opts.length = 1;
			...
			for(var i=0; i<ids.length-1;i++) {
				var sid = ids[i+1]
				$("#" + ids[i]).attr('next-id', sid);
				var nextids = '';
				for(var j=i+1;j<ids.length;j++) {
					nextids += ids[j] + ",";
				}
				$("#" + ids[i]).attr('next-ids', nextids);
				$("#" + ids[i]).bind('change', function(){
					TRIG.dict.setSelectByParentAndCode($(this).attr('next-id'), $(this).find("option:selected").attr('data-id'), '');
					var _ids = $(this).attr('next-ids');
					if(_ids) {
						var nextids= _ids.split(",");
						for(var i=0;i<nextids.length;i++) {
							if(nextids[i]) {
								$("#" + nextids[i]).get(0).options.length=1;
							}
						}
					}
				});
		...
		updateSelectValueByCode :function(ids,NO,code) {
			if(code) {
				$.getJSON(TRIG.PATH + '/dictionary_findsByCode.json', 'sd_dictionaryCode='+code+'&sd_dictionaryNo=' + NO, function(data){
					if(data.rows.length>0) {
						for(var i=0;i<ids.length;i++) {
							$("#"+ids[i]).attr('data-value', data.rows[i]);
						}
						$("#"+ids[0]).val(data.rows[0]);
						$("#"+ids[0]).change();
					}
				});
			}
...
TRIG.Datatable = function(opts) {
	...
	var tablesetting = {   
            ...
            "sDom": "<'row'<'col-md-6 col-sm-12'><'col-md-6 col-sm-12'f>r><'table-scrollable't><'row'<'col-md-5 col-sm-12'li><'col-md-7 col-sm-12'p>>"
	    };
...
// 部门多选
TRIG.OrgMultipleSelect = function(id, multiple) {
	var jq = $("#" + id);
	var _multiple = multiple==false ?  false : true;
	var selectobj = jq.select2({
        minimumInputLength: 0,   
        placeholder: "请选择",
        multiple :  _multiple,
         ajax: {  
             url: TRIG.PATH + '/organization_multipleSelect.json',
             dataType: 'json',
             data: function (term, page) {
                 return {
                	so_organizationName: term, 
                 	rows:15 
                 };
             },
             results: function (data, page) {
             	...
             	for(var i=0;i<data.length&&i<15;i++) {
             		datas.push({id:data[i].so_organizationId,text:data[i].so_organizationName});
             	}
                 return {
                     results: datas 
                 };
	...
	return { 
		...
		setVal : function(vals) {
			if(vals) {
				$.getJSON(TRIG.PATH + "/organization_finds.json", "ids=" + vals, function(data){
					var datas = [];
					for(var i=0;i<data.length;i++){
						datas.push({id:data[i].so_organizationId, text:data[i].so_organizationName});
					}
...
TRIG.SELECT = {							
	config : {
        ...
        individualParam: {}, // 个性化静态参数，用于更多个性化查询条件，JSON对象
        actionParam : function() { // 个性化动态参数
        	return {};
        }
	},
